...
def rows_for(stem, lab2):
    rows = []
    for k in range(1, lab2.max() + 1):
        m = lab2 == k
        if m.sum() == 0: continue
        e = mu.encode(np.asfortranarray(m.astype(np.uint8)))
        rows.append({'filament_id': f'{stem}_{len(rows) + 1}', 'segmentation_rle': e['counts'].decode('ascii')})
    return rows
...
def main():
    ...
    ap.add_argument('--imgs', default='/home/user/work/test1024'); ap.add_argument('--out', required=True)
    a = ap.parse_args()
    ...
    for f in files:
        ...
        if F is not None and lab.max() > 0:
            ...
            keep = F['model'].predict(inst_features(lab, p[0], p[1], img)) > a.thr
            lut = np.zeros(lab.max() + 1, np.int32); lut[1:][keep] = np.arange(1, keep.sum() + 1); lab = lut[lab]
        rows += rows_for(f.stem, to2048(lab, p[0], cfg['t']))
    df = pd.DataFrame(rows, columns=['filament_id', 'segmentation_rle']); df.to_csv(a.out, index=False)
